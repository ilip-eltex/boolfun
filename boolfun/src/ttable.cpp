...
namespace bf
{
    ...
    ttable::ttable(vector<vector<bvect64> > anf_db)
    {
        ...
        for (uint64_t i = 0; i < anf_db.size(); i++)
        { // container for (containers for bits in values vector
            current_value = 0;
            for (uint64_t j = 0; j < anf_db[i].size(); j++)
            { // anf_db[j] - container for bit number j in values vector
                for (int k = 0; k < 63; k++)
                {

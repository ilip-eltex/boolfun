...
namespace bf
{
    ...
    polynom::polynom(ttable &t, GF *field)
    {
        ...
        const uint32_t tlen = t.get_length();

        for (uint32_t i = 0; i < tlen; i++)
            values.push_back(t.get_value(i));

        for (uint32_t coeffq = 1; coeffq < this->_length; coeffq++)
            for (uint32_t valuesq = 0; valuesq < tlen; valuesq++)
                this->_coeff[coeffq] += field->multiply(t.get_value(valuesq), field->power(valuesq, (tlen - valuesq - 1)));
    }
